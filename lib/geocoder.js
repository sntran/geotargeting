...
module.exports = function ( options ) {
  ...
  return function *geocoder ( next ) {
    ...
    if ( address ) {
      ...
      if (data.error_message && !location) {
        console.error(data.error_message);
        return;
      }

      var coordinates = location.geometry.location;
      debug( 'Geocoded input', address, 'into', location.formatted_address, 'at', coordinates.lat, coordinates.lng);
